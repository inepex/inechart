...
public class Circle extends Shape {
	...
	public Circle(int radius, ShapeProperties properties) {
		super(properties);
		this.radius = radius;
	}
	...
	ArrayList<GraphicalObject> toGraphicalObjects() {
		if(properties.getLineProperties().getLineWidth() > 0){
			...
			outer = new com.inepex.inegraphics.impl.client.ishapes.Circle(
					0, 0, 
					this.zIndex,
					outerContext, true, false, this.radius);	
		}
		if(properties.getFillColor() != null){
			Context innerContext = new Context(
					...
					shadowColor == null ? 0d : shadowColor.getAlpha(),
					shadowColor == null ? Color.DEFAULT_COLOR : shadowColor.getColor());
			inner = new com.inepex.inegraphics.impl.client.ishapes.Circle(
						0, 0, 
						this.zIndex,
